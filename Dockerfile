FROM hasura/graphql-engine:v1.2.1



## Comment the command above and use the command below to
## enable an access-key and an auth-hook
## Recommended that you set the access-key as a environment variable in heroku
#CMD graphql-engine \
#    --database-url $DATABASE_URL \
#    serve \
#    --server-port $PORT \
#    --access-key XXXXX \
#    --auth-hook https://myapp.com/hasura-webhook 
#
FROM hasura/graphql-engine:latest

ENV HASURA_GRAPHQL_DATABASE_URL postgres://postgres:bragapoin@hello-hasura.coz1kemlydbm.ap-southeast-1.rds.amazonaws.com:5432/hello-hasura

ENV HASURA_GRAPHQL_ADMIN_SECRET myadminsecret

ENV HASURA_GRAPHQL_ENABLE_CONSOLE true

EXPOSE 80

CMD ["graphql-engine", "serve","--enable-console"]